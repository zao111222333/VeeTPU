...
class compare2(dataWidth:Int) extends BlackBox {
  ...
  val io=new Bundle{
    val i = new Bundle {
      val X  = in Vec(Bits (dataWidth bit), 2)
    }
    val o = new Bundle {
      val Max = out Bits (dataWidth bit)
    }
  }
  noIoPrefix()
  addRTLPath("./src/main/scala/fp/compare/verilog/compare2.v")
}

class compare16(dataWidth:Int) extends Component {
  val io=new Bundle{
    val i = new Bundle {
      val X  = in Vec(Bits (dataWidth bit), 16)
    }
    val o = new Bundle {
      val Max = out Bits (dataWidth bit)
      val Shift = out Vec(Bits (dataWidth bit), 16)
    }
  }
  noIoPrefix()
  val sim = Reg(Bool)
  ...
  for(i <- 0 to 16-1){
    io.o.Shift(i) := (compareArray(3)(0).io.o.Max.asUInt - io.i.X(i).asUInt).asBits
  }
