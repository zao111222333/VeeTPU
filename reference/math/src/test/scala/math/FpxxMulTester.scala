...
object FPMulTester {
    ...
    class FPMulDut(config: FPConfig) extends Component {
        val io = new Bundle {
            val op_vld      = in(Bool)
        ...
        val fp_op = new FPMul(config, FPMulConfig(pipeStages = 5))
...
class FPMulTester extends FunSuite {
...
//            .withWave
            .allOptimisation
            .compile(new FPMulTester.FPMulDut(config))
        ...
        compiled.doSim { dut =>
            ...
            val stimuli = FPTesterSupport.directedStimuli
