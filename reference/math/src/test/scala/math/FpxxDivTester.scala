...
object FPDivTester {
    ...
    class FPDivDut(config: FPConfig) extends Component {
        val io = new Bundle {
            val op_vld      = in(Bool)
        ...
        val fp_op = new FPDiv(config, FPDivConfig(pipeStages = 5) )
...
class FPDivTester extends FunSuite {
...
//            .withWave
            .compile(new FPDivTester.FPDivDut(config))
