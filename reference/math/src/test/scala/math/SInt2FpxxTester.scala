...
object SInt2FPTester {

    class SInt2FPDut(config: FPConfig) extends Component {
        val io = new Bundle {
            val op_vld      = in(Bool)
        ...
        u_sint2FP_8.io.op_vld := RegNext(io.op_vld) init(False)
        u_sint2FP_8.io.op     := RegNext(io.op_a.asSInt) init(0)

        io.result_vld := RegNext(u_sint2FP_8.io.result_vld) init(False)
        io.result_a   := RegNext(u_sint2FP_8.io.result).toVec()

        val u_sint2FP_28 = new SInt2FP(28, config)
        u_sint2FP_28.io.op_vld := RegNext(io.op_vld) init(False)
        u_sint2FP_28.io.op     := RegNext(io.op_b.asSInt) init(0)

        io.result_b   := RegNext(u_sint2FP_28.io.result).toVec()
    }
}

class SInt2FPTester extends FunSuite {
    ...
    test("SInt2FP") {
        ...
        var compiled = SimConfig
            .withWave
            .compile(new SInt2FPTester.SInt2FPDut(config))
