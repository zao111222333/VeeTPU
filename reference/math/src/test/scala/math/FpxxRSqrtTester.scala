...
object FPRSqrtTester {

    class FPRSqrtDut(config: FPConfig) extends Component {
        val io = new Bundle {
            val op_vld      = in(Bool)
        ...
        val fp_op = new FPRSqrt(config, FPRSqrtConfig(pipeStages = 5) )
...
class FPRSqrtTester extends FunSuite {
    ...
    test("FPRSqrt") {
        ...
        var compiled = SimConfig
            .withWave
            .compile(new FPRSqrtTester.FPRSqrtDut(config))
