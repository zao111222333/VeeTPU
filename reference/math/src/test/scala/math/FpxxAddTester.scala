...
object FPAddTester {
    ...
    class FPAddDut(config: FPConfig) extends Component {
        val io = new Bundle {
            val op_vld      = in(Bool)
        ...
        val fp_op = new FPAdd(config, FPAddConfig(pipeStages = 5))
...
class FPAddTester extends FunSuite {
...
//            .withWave
            .allOptimisation
            .compile(new FPAddTester.FPAddDut(config))
