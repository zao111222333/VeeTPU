...
object FPSqrtTester {
    ...
    class FPSqrtDut(config: FPConfig) extends Component {
        val io = new Bundle {
            val op_vld      = in(Bool)
        ...
        val fp_op = new FPSqrt(config, FPSqrtConfig(pipeStages = 5) )
...
class FPSqrtTester extends FunSuite {
...
//            .withWave
            .compile(new FPSqrtTester.FPSqrtDut(config))
