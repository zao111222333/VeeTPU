...
#define FP_SQRT_LUT_SIZE_BITS          8
#define FP_SQRT_LUT_SIZE               ((1L<<FP_SQRT_LUT_SIZE_BITS)-(1L<<(FP_SQRT_LUT_SIZE_BITS-2)))
#define FP_SQRT_LUT_MANT_BITS          12
#define FP_SQRT_LUT_SHIFT_BITS         4
#define FP_SQRT_FRAC_BITS              8
...
public:
    ...
    friend FP<_m_size, _exp_size, _zero_offset>  operator+(const FP<_m_size, _exp_size, _zero_offset> left, const FP<_m_size, _exp_size, _zero_offset> right) {
        FP<_m_size, _exp_size, _zero_offset> r;
    ...
    friend bool operator< (const FP<_m_size, _exp_size, _zero_offset> left, const FP<_m_size, _exp_size, _zero_offset> right) {
    ...
    friend FP<_m_size, _exp_size, _zero_offset> sqrt(const FP<_m_size, _exp_size, _zero_offset> op){

        static bool init = false;
        static sqrt_lut_entry_t sqrt_lut[FP_SQRT_LUT_SIZE];

        if (!init){
            int max_shift = 0;
            for(int i=0;i<FP_SQRT_LUT_SIZE;++i){
                double fin     = (double)((1L<<(FP_SQRT_LUT_SIZE_BITS-2)) + i)/(1L<<(FP_SQRT_LUT_SIZE_BITS-1));
                ...
                sqrt_lut[i].mant  = mant >> (FP64_MANT_BITS-FP_SQRT_LUT_MANT_BITS);
        ...
        lut_addr = lut_addr >> (_m_size+2-FP_SQRT_LUT_SIZE_BITS);
        lut_addr = lut_addr - (1L<<(FP_SQRT_LUT_SIZE_BITS-2));
        ...
        r.m     = lut_val.mant << (_m_size-FP_SQRT_LUT_MANT_BITS);

        return r;
    }

    friend FP<_m_size, _exp_size, _zero_offset> recip_sqrt(const FP<_m_size, _exp_size, _zero_offset> op){
    ...
    FP<_m_size, _exp_size, _zero_offset> abs () const {
        FP<_m_size, _exp_size, _zero_offset> r;
