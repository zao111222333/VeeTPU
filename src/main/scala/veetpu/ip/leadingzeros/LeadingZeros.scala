...
class LeadingZeros(dataWidth: Int) extends Component{
    val io = new Bundle{
        val i = in Bits(dataWidth bits)
        val o = out UInt(log2Up(dataWidth+1) bits)
    }
    io.o := LeadingZeros_(io.i)
}