...
class FloatingAdder(fpatConfig: FPATConfig) extends Component{
  import fpatConfig._
...
// 得到hasFlag
  var pipe_0 = new Area{
    val stageModule = new Area{
      val compare = new compare2_diff(fpConfig.expSize)
...
// TODO : adderTree 内部csla的设置需要每次都手动调整
  ...
  var pipe_1 = new Area{
    ...
    val stageModule = new Area{
      ...
      val adderTree = new AdderTree2S(fpConfig.fullMantSize + accuracy, adderTree_CLSA_PRE_WIDTH) //sign 1bit
    }
...
object FloatingAdder_Verilog {
  def main(args: Array[String]): Unit = {
    ...
    val path = "simWorkspace/FloatingAdder/rtl"
    java.nio.file.Files.createDirectories(java.nio.file.Paths.get(path))
    val report = SpinalConfig(
      mode=Verilog,
      targetDirectory=path
    ).generate(new FloatingAdder(fpatConfig))
