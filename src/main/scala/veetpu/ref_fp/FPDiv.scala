...
class FPDiv(c: FPConfig, divConfig: FPDivConfig = null) extends Component {

    assert((c.mantSize&1)==1, "FPDiv: mantissa must be odd")

    def pipeStages      = if (divConfig == null) 0 else divConfig.pipeStages
    def halfBits        = (c.mantSize+1)/2
    ...
    val div_addr_p0 = op_b_p0.mant >> (c.mantSize-tableSizeBits)

    val exp_p0      = op_a_p0.exp.resize(c.expSize+1).asSInt - op_b_p0.exp.resize(c.expSize+1).asSInt
    val sign_p0     = op_a_p0.sign ^ op_b_p0.sign

    val op_a_zero_p0 = op_a_p0.isZero()
    val op_b_zero_p0 = op_b_p0.isZero()
    val op_a_inf_p0  = op_a_p0.isInfinite()
    val op_b_inf_p0  = op_b_p0.isInfinite()
    val op_nan_p0    = op_a_p0.isNan() || op_b_p0.isNan() || (op_a_inf_p0 && op_b_inf_p0)
    ...
    val exp_full_p1  = exp_p1.resize(c.expSize+2) - recip_exp_p1.resize(c.expSize+2).asSInt + S(c.bias+1, c.expSize+2 bits)
    ...
    val div_adj_p5 = UInt(c.mantSize bits)
    val exp_adj_p5 = SInt(c.expSize+2 bits)
    ...
    val exp_final_p6  = UInt(c.expSize bits)
    val div_final_p6  = UInt(c.mantSize bits)

    when((op_a_zero_p6 && op_b_zero_p6) || op_nan_p6){
        // 0/0 -> Nan
        sign_final_p6 := sign_p6
        exp_final_p6.setAll
        div_final_p6  := (c.mantSize-1 -> True, default -> False)
    }
    .elsewhen(exp_adj_p6 >= ((1<<c.expSize)-1) || op_b_zero_p6){
        // Infinity
        sign_final_p6 := sign_p6
        exp_final_p6.setAll
    ...
    .otherwise{
        sign_final_p6 := sign_p6
        exp_final_p6  := exp_adj_p6(0, c.expSize bits).asUInt
