...
class SInt2FP(intNrBits: Int, c: FPConfig) extends Component {
    ...
    .otherwise{
        ...
        if (intNrBits-1 >= c.mantSize) {
            mant_final_p2 := op_adj_p2(intNrBits-c.mantSize-1, c.mantSize bits)
        }
        else{
            mant_final_p2 := op_adj_p2.resize(intNrBits-1) @@ U(0, (c.mantSize - intNrBits + 1) bits)
        }
    }
