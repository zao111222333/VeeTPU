...
class FPDemo extends Component {
    ...
    val io = new Bundle {
        ...
        val op_vld      = in(Bool)
        val op_a        = in(Bits(config.fullSize bits))
        val op_b        = in(Bits(config.fullSize bits))

        val op_a_p_op_b_vld     = out(Bool)
        val op_a_p_op_b         = out(Bits(config.fullSize bits))

        val op_a_mul_op_b_vld   = out(Bool)
        val op_a_mul_op_b       = out(Bits(config.fullSize bits))

        val op_a_div_op_b_vld   = out(Bool)
        val op_a_div_op_b       = out(Bits(config.fullSize bits))
    }
    ...
    val core = new ClockingArea(coreClockDomain) {
        ...
        add.io.op_a.fromBits(RegNext(io.op_a))
        add.io.op_b.fromBits(RegNext(io.op_b))

        io.op_a_p_op_b_vld := RegNext(add.io.result_vld)
        io.op_a_p_op_b     := RegNext(add.io.result).asBits()

        val mul = new FPMul(config, FPMulConfig(pipeStages = 5))
        mul.io.op_vld :=    RegNext(io.op_vld)
        mul.io.op_a.fromBits(RegNext(io.op_a))
        mul.io.op_b.fromBits(RegNext(io.op_b))

        io.op_a_mul_op_b_vld := RegNext(mul.io.result_vld)
        io.op_a_mul_op_b     := RegNext(mul.io.result).asBits()

        val div = new FPDiv(config, FPDivConfig(pipeStages = 4, tableSizeBits = -1))
        div.io.op_vld :=    RegNext(io.op_vld)
        div.io.op_a.fromBits(RegNext(io.op_a))
        div.io.op_b.fromBits(RegNext(io.op_b))

        io.op_a_div_op_b_vld := RegNext(div.io.result_vld)
        io.op_a_div_op_b     := RegNext(div.io.result).asBits()
    }
}
