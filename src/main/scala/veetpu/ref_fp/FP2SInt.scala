...
class FP2SInt(intNrBits: Int, fracNrBits: Int, c: FPConfig) extends Component {
    
    assert(intNrBits + fracNrBits + 2 > c.mantSize, "Not enough bits for SInt size")
    ...
    val shift_p0    = SInt(c.expSize+2 bits)
