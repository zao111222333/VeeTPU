...
class FPRSqrt(c: FPConfig, rsqrtConfig: FPRSqrtConfig = null) extends Component {

    def pipeStages      = if (rsqrtConfig == null) 0 else rsqrtConfig.pipeStages
    def lutMantBits     = if (rsqrtConfig == null || rsqrtConfig.lutMantBits < 0)   c.mantSize   else rsqrtConfig.lutMantBits
    def tableSizeBits   = if (rsqrtConfig == null || rsqrtConfig.tableSizeBits < 0) c.mantSize/2 else rsqrtConfig.tableSizeBits
    ...
    val op_zero_p0 = op_p0.isZero()
    val op_nan_p0  = op_p0.isNan() || op_p0.sign
    val op_inf_p0  = op_p0.isInfinite()

    val exp_p0 = SInt(c.expSize+1 bits)
    exp_p0 := op_p0.exp.resize(c.expSize+1).asSInt - c.bias

    val gt_1_p0 = !(exp_p0).lsb

    val rsqrt_addr_p0 = UInt(tableSizeBits bits)
    rsqrt_addr_p0 := (((U(1,1 bits) @@ op_p0.mant) << gt_1_p0.asUInt) >> (c.mantSize+2-tableSizeBits)) - (1<<(tableSizeBits-2))
    ...
    val exp_adj_p1 = SInt(c.expSize+1 bits)
    exp_adj_p1 := -((exp_p1+1) |>> 1) - rsqrt_shift_p1 + c.bias

    val sign_final_p1 = Bool
    val exp_final_p1  = UInt(c.expSize bits)
    val mant_final_p1 = UInt(c.mantSize bits)

    when(op_nan_p1){
        // Negative -> NaN
        sign_final_p1   := False
        exp_final_p1.setAll
        mant_final_p1   := (c.mantSize-1 -> True, default -> False)
    }
    .elsewhen(op_zero_p1){
        // Infinity
        sign_final_p1   := False
    ...
    .otherwise{
        sign_final_p1   := False
        exp_final_p1    := exp_adj_p1.asUInt.resize(c.expSize)
        mant_final_p1   := rsqrt_mant_p1 << (c.mantSize-lutMantBits) 
    }
